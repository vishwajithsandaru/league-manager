...
export class UniversityLeagueService {
  ...
  public getLeagueTable(season: string): Observable<LeagueTable[]>{
    ...
    let tableDetails = this.http.get<LeagueTable[]>("/api/university/display-league-table/"+seasonYear);
    return tableDetails;
  }

  public addRandomMatch(season: String): Observable<MatchResponse>{
    let seasonYear = season.slice(0, -3);
    let matchRes = this.http.post<MatchResponse>("api/university/add-random-match/"+seasonYear, "");
    return matchRes;
  }
